...
const CLOUDINARY_CLOUD_NAME = 'doajvqv3f'
const getCloudinaryUrl = (imageUrl, options = {}) => {
  if (!imageUrl || typeof imageUrl !== 'string') return imageUrl
  if (imageUrl.startsWith('data:')) return imageUrl
  const { width = 2560, height = 960, quality = 'auto', format = 'auto' } = options
  const transformations = `w_${width},h_${height},c_fill,q_${quality},f_${format}`
  return `https://res.cloudinary.com/${CLOUDINARY_CLOUD_NAME}/image/fetch/${transformations}/${encodeURIComponent(imageUrl)}`
}
