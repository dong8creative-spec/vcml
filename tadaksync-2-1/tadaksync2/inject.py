"""자막을 캡컷 프로젝트(맥: draft_info.json)에 텍스트 트랙으로 삽입.

캡컷이 실제로 생성하는 자막 구조(native_text_schema의 실측 템플릿)를 그대로
복제해 삽입한다. 맥 CapCut 8.9.1의 텍스트 소재 구조가 이 템플릿과 사실상
동일함을 실측으로 확인했다(필드 1개씩만 상이, 무해).

주의: 맥 캡컷은 프로젝트 최상위 draft_info.json 외에 template-2.tmp 사본과
'Timelines/<내부 id>/draft_info.json' 사본을 함께 유지한다(세 파일 MD5 동일).
편집기가 어느 것을 읽어도 자막이 보이도록 존재하는 사본을 모두 갱신한다.

v2 확장: 스타일 프리셋(styles.py)을 위해 그림자·배경 박스·어절 강조(span)를 지원한다.
"""
...
from .capcut import DRAFT_BASENAMES, draft_file
...
def backup_draft(draft_dir: Path) -> Path:
    """초안 타임라인 파일(draft_info.json) 백업본 생성."""
    src = draft_file(draft_dir)
    if src is None:
        raise FileNotFoundError(
            f"초안 파일({' 또는 '.join(DRAFT_BASENAMES)})이 없습니다: {draft_dir}")
    stamp = time.strftime("%Y%m%d_%H%M%S")
    dst = draft_dir / f"{src.stem}.aisub_backup_{stamp}.json"
    shutil.copy2(src, dst)
    backups = sorted(draft_dir.glob(f"{src.stem}.aisub_backup_*.json"))
    for old in backups[:-10]:
        old.unlink(missing_ok=True)
    return dst


def _sibling_copies(draft_dir: Path, content_id: str, primary: Path) -> list[Path]:
    """캡컷 편집기가 함께 읽는, 같은 내용의 사본 파일 경로들."""
    copies: list[Path] = []
    tmp = draft_dir / "template-2.tmp"
    if tmp.is_file():
        copies.append(tmp)
    if content_id:
        tl = draft_dir / "Timelines" / content_id / primary.name
        if tl.is_file():
            copies.append(tl)
        else:
            tl_dir = draft_dir / "Timelines" / content_id
            for name in (*DRAFT_BASENAMES, "template-2.tmp"):
                cand = tl_dir / name
                if cand.is_file() and cand not in copies:
                    copies.append(cand)
    return copies
...
def _system_font_path(data: dict) -> str:
    ...
    for bundle in app_bundles:
        font_dir = bundle / "Contents" / "Resources" / "Font" / "SystemFont"
        if not font_dir.is_dir():
            continue
        en = font_dir / "en.ttf"
        if en.is_file():
            return str(en)
        candidates = sorted(font_dir.glob("*.ttf"))
        if candidates:
...
def inject_subtitles(draft_dir: Path, lines: list[SubtitleLine],
    ...
    content_path = draft_file(draft_dir)
    ...
    sibling_paths = _sibling_copies(draft_dir, data.get("id", ""), content_path)

    sibling_backups: list[tuple[Path, Path]] = []
    stamp = time.strftime("%Y%m%d_%H%M%S")
    for sp in sibling_paths:
        b = sp.with_name(f"{sp.stem}.aisub_backup_{stamp}{sp.suffix}")
        shutil.copy2(sp, b)
        sibling_backups.append((sp, b))
    ...
    try:
        ...
        for sp in sibling_paths:
            sp.write_text(serialized, encoding="utf-8")
    except Exception:
        shutil.copy2(backup, content_path)
        for sp, b in sibling_backups:
            shutil.copy2(b, sp)
        raise
    return backup
