...
SR = 16000
CAPCUT_FPS = 30
SUBTITLE_START_FRAME_PAD = 4  # 30fps ≈133ms — 온셋 미검출 시 Whisper보다 늦춤


def subtitle_start_pad_us() -> int:
    return US * SUBTITLE_START_FRAME_PAD // CAPCUT_FPS
...
class Transcriber:
    ...
    def transcribe(
        ...
        progress("자막 시작 시각을 오디오에 맞추고 있어요...")
        aligned = align_line_starts_to_audio(lines, audio)
        return _close_gaps(aligned)
...
def _block_speech_start(
    ...
    """블록 하나의 자막 시작(µs) — 실제 발화 온셋 우선, Whisper는 힌트만."""
    pad_us = subtitle_start_pad_us()
    min_dur_us = 200_000
    ...
    # 쉼 뒤 새 발화 → VAD 구간 시작 시각
    if gap >= 0.12:
        for rs, re in regions:
            if re <= prev + 0.02:
                continue
            if rs <= w + 0.2 and re > prev + 0.02:
                start_sec = max(rs, prev + 0.01)
                break

    # RMS 온셋 (Whisper가 늦어도 넓게 탐색)
    if start_sec is None:
        lo = max(prev + 0.01, w - 0.45)
        hi = min(end, w + 0.15, audio_dur)
        if hi > lo + 0.02:
            onset = find_onset(audio, lo, hi, env)
            if onset is not None:
                start_sec = max(onset, prev + 0.01)

    # 같은 호흡 안 다음 블록 → 해당 어절 온셋만 좁게 탐색
    if start_sec is None and gap < 0.12:
        lo = max(prev + 0.01, w - 0.15)
        hi = min(end, w + 0.1, audio_dur)
        if hi > lo + 0.02:
            onset = find_onset(audio, lo, hi, env)
            if onset is not None:
                start_sec = max(onset, prev + 0.01)
    ...
    if start_sec is None:
        if gap < 0.12:
            start_sec = max(w, prev + 0.01)
        else:
            start_sec = w + pad_us / US

    start_us = max(int(start_sec * US), prev_end_us)
    if end_us - start_us < min_dur_us:
        start_us = max(prev_end_us, end_us - min_dur_us)
    return start_us
