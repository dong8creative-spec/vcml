...
def app_data_dir() -> Path:
    """로그인/기기 정보 저장 폴더 (맥: ~/Library/Application Support/TadakSync)."""
    ...
    if not current.exists():
        for legacy in (base / LEGACY_APP_DIR_NAME / AUTH_FILE,
                       Path.home() / ".config" / APP_DIR_NAME / AUTH_FILE,
                       Path.home() / ".config" / LEGACY_APP_DIR_NAME / AUTH_FILE):
            if legacy.exists():
                try:
