"""캡컷 프로젝트(draft) 탐색 및 타임라인 오디오 재구성 — 맥 체험판.

캡컷 맥 버전은 프로젝트 타임라인을 draft_info.json에 저장한다.
트랙/세그먼트를 분석하여 타임라인과 동일한 16kHz 모노 오디오를 만든다.
...
# 맥은 draft_info.json, 없으면 윈도우형 draft_content.json
...
def _custom_draft_paths() -> list[Path]:
    """사용자 지정 초안 위치(맥). 기본 위치 + 수동 등록에 의존한다."""
    return []


def _app_data_dir() -> Path:
    folder = Path.home() / "Library" / "Application Support" / "TadakSyncTrial"
...
def list_projects() -> list[Project]:
    """초안 타임라인 파일이 있는 캡컷 프로젝트를 최근 수정순으로 나열."""
