...
로그인·코인 없이 완전 무료로 동작합니다(전사·번역·삽입 모두 로컬).
설정은 `%APPDATA%\TadakSync3`에 저장되고, `tadaksync-v2`와 완전히 별개로 실행됩니다.

## v3에서 새로 추가된 것

- **라이트 + 오렌지 테마** (v2는 다크 + 라임).
- **자막 블록 삭제 시 영상 클립 리플 삭제.** 4단계 블록 목록에서
  - `✕` = 자막만 삭제 (기존 동작)
  - `✂` = 그 블록 `[start, end)` 구간의 영상 클립까지 삭제. 삽입할 때 모든
    트랙(영상·오디오·자막·효과)에서 그 구간을 잘라내고 뒤쪽 세그먼트를 잘린
    길이만큼 앞으로 당깁니다(리플 삭제). 남은 자막 시각도 새 타임라인에 맞춰
    재계산됩니다. `draft_content.json`은 삽입 전 자동 백업됩니다.
  - 한계: 세그먼트 경계에 걸친 클립은 잘라 붙이고 그 구간 키프레임은 버립니다.
    겹침 트랜지션·복합 클립 내부 타임라인까지 완벽히 재계산하지는 않습니다.
    구현: `tadaksync3/capcut.py` 의 `ripple_delete_ranges()`.
...
`tadaksync3/web/index.html?dev=1` — 로그인·인식 흐름만 mock으로 확인
...
```powershell
npm run build:subtitle-tool      # PyInstaller → dist/TadakSync3
...
- `tadaksync3/app.py` — pywebview 창  
- `tadaksync3/api.py` — JS ↔ Python 브리지  
- `tadaksync3/keyword_spans.py` — 키워드 일괄 강조  
- `tadaksync3/styles.py` — 자막 스타일  
- `tadaksync3/web/` — UI  
- `tadaksync3/license.py` — 로그인·코인·스마트스토어 후기 API  
