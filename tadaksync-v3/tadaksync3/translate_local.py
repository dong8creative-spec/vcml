...
def _get_argos_translation(from_code: str, to_code: str):
    ...
    try:
        ...
        raise RuntimeError(
            "번역 패키지가 없어요. tadaksync3 폴더에서 "
            "`pip install -r requirements.txt` 후 "
            "`python -m tadaksync3.translate_local --install` 을 실행해 주세요."
        ) from e
    ...
    if tr is None:
        raise RuntimeError(
            ...
            "`python -m tadaksync3.translate_local --install` 로 언어팩을 설치해 주세요."
        )
...
if __name__ == "__main__":
    ...
    if "--install" in sys.argv:
        ...
        print("사용법: python -m tadaksync3.translate_local --install")
