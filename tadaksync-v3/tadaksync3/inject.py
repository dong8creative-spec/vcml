...
def _apply_clip_cuts(data: dict, lines: list[SubtitleLine],
                     translated_lines: list[SubtitleLine] | None,
                     clip_delete_ranges: list[tuple[int, int]] | None,
                     ) -> tuple[list[SubtitleLine], list[SubtitleLine] | None, int]:
    """clip_delete_ranges 구간을 프로젝트 타임라인에서 리플 삭제하고,
    남은 자막 라인들의 시각을 삭제 후 타임라인에 맞춰 다시 계산한다."""
    merged = capcut.merge_ranges([(int(s), int(e)) for s, e in
                                  (clip_delete_ranges or [])])
    if not merged:
        return lines, translated_lines, 0

    removed = capcut.ripple_delete_ranges(data, merged)

    def _remap(seq: list[SubtitleLine] | None) -> list[SubtitleLine] | None:
        if not seq:
            return seq
        out: list[SubtitleLine] = []
        for l in seq:
            ns = capcut.remap_time_through_deletions(l.start_us, merged)
            ne = capcut.remap_time_through_deletions(l.end_us, merged)
            if ne - ns >= 1000:            # 1ms 미만으로 뭉개지면 버림
                out.append(SubtitleLine(start_us=ns, end_us=ne, text=l.text,
                                        spans=getattr(l, "spans", []) or []))
        return out

    return _remap(lines) or [], _remap(translated_lines), removed
...
def inject_subtitles(
    ...
    *,
    ...
    clip_delete_ranges: list[tuple[int, int]] | None = None,
) -> Path:
    """자막 라인을 캡컷 프로젝트에 삽입. 백업 파일 경로를 반환.

    inject_mode: original | translated | both
    clip_delete_ranges: [(start_us, end_us), ...] — 이 구간의 영상 클립을
        리플 삭제(뒤 전체 당김)한 뒤 자막을 넣는다. (타닥싱크 3 기능)
    """
    ...
    try:
        ...
        lines, translated_lines, _removed_us = _apply_clip_cuts(
            data, lines, translated_lines, clip_delete_ranges)
