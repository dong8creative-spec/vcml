...
function syncTranslateLangUI() {
  $$("#seg-translate-lang button").forEach((b) => {
    b.classList.toggle("on", (b.dataset.v || "") === (state.translateLang || ""));
  });
  updateTranslateEstimate();
}

function requestTranslate() {
  if (!state.translateLang) {
    toast("번역할 언어를 먼저 선택해 주세요.", "warn");
    return;
  }
...
async function doTranslate() {
  $("#translate-overlay").classList.add("hidden");
  if (!state.translateLang || !state.blocks.length) return;
  const btn = $("#btn-run-translate");
  state.busy = true;
  btn.disabled = true;
  ...
  try {
    ...
    if (typeof res.balance === "number") {
      state.auth = { ...(state.auth || {}), balance: res.balance, logged_in: true };
      renderAuth();
    }
    if (state.injectMode === "original") state.injectMode = "both";
    syncInjectModeUI();
    renderBlocks();
    toast(`${res.target_language_label || "번역"} 완료! (${res.coins || 0}코인 차감)`, "success");
  } finally {
    state.busy = false;
...
function renderBlocks() {
  ...
  list.innerHTML = "";
  const hasTr = state.blocks.some((b) => (b.text_translated || "").trim());
  state.blocks.forEach((b, i) => {
    if (!Array.isArray(b.spans)) b.spans = [];
    const row = document.createElement("div");
    row.className = "block-row" + (state.playingIdx === i ? " playing" : "") + (hasTr ? " dual" : "");
    row.innerHTML = `
      ...
      <div class="block-texts">
        <input class="block-text" value="${esc(b.text)}" placeholder="원어">
        ${hasTr || state.translateLang
          ? `<input class="block-text translated" value="${esc(b.text_translated || "")}" placeholder="번역어">`
          : ""}
      </div>
    ...
    const trInput = row.querySelector(".block-text.translated");
    if (trInput) {
...
function renderInjectSummary() {
  ...
  const modeLabel = {
    ...
    translated: "번역만",
    both: "원어+번역",
  }[state.injectMode] || state.injectMode;
  let extra = modeLabel;
  if ((state.injectMode === "translated" || state.injectMode === "both") && !hasTr) {
    extra += " (번역 필요)";
  }
  $("#inject-summary").textContent =
    `「${name}」에 자막 ${state.blocks.length}개 · ${extra}`;
}

function syncInjectModeUI() {
  $$("#seg-inject-mode button").forEach((b) => {
    b.classList.toggle("on", b.dataset.v === state.injectMode);
  });
  renderInjectSummary();
}
...
async function requestInject() {
  ...
  const hasTr = state.blocks.some((b) => (b.text_translated || "").trim());
  if ((state.injectMode === "translated" || state.injectMode === "both") && !hasTr) {
    toast("번역 자막이 없어요. 4단계에서 번역하거나 원어만 삽입해 주세요.", "warn");
    gotoStep(4);
    return;
  }
...
async function doInject() {
  ...
  try {
    ...
    const modeNote = {
      ...
      translated: "번역 트랙",
      both: "원어+번역 트랙",
    }[res.inject_mode || state.injectMode] || "자막";
    $("#done-msg").textContent =
      `「${res.project}」에 ${modeNote} ${res.count}개를 삽입했어요.`;
...
async function openHistory() {
  ...
  list.innerHTML = res.history.map((h) => {
    ...
    const plus = delta > 0;
    const desc = h.description || reasonMap[h.reason] || h.reason || h.type || "";
    return `<div class="history-row">
      <span class="h-when">${esc(h.created_at || h.when || "")}</span>
      <span class="h-desc">${esc(desc)}${h.minutes ? ` (${h.minutes}분)` : ""}</span>
...
function makeMockApi() {
  ...
  return {
    get_state: async () => ({
      ok: true,
      app: { name: "타닥싱크 3", version: "3.0.0-mock" },
      auth: mockAuth,
    ...
    translate_blocks: async (blocks, targetLang, durationUs) => {
      ...
      const fn = dict[targetLang] || ((t) => t);
      return {
        ok: true,
        blocks: blocks.map((b) => ({ ...b, text_translated: fn(b.text) })),
        minutes: mins,
        coins,
        target_lang: targetLang,
        target_language_label: TRANSLATE_LABELS[targetLang] || targetLang,
        balance: mockAuth.balance,
      };
    },
    inject: async (blocks, _style, _size, _pos, projectIndex, injectMode = "both") => {
      if (projectIndex == null) return { ok: false, error: "프로젝트를 다시 선택해 주세요." };
      return {
        ok: true,
        count: blocks.length * (injectMode === "both" ? 2 : 1),
        project: ["0714 브이로그 최종", "쇼츠 - 자막 실험", "강의 3강 편집본", "0627_test4"][projectIndex] || "프로젝트",
        backup: "C:\\mock\\backup.json",
...
function bindEvents() {
  ...
  $("#btn-go-style").addEventListener("click", () => {
    ...
    const hasTr = state.blocks.some((b) => (b.text_translated || "").trim());
    if (!hasTr && (state.injectMode === "translated" || state.injectMode === "both")) {
  ...
  $("#seg-inject-mode").addEventListener("click", (e) => {
    const btn = e.target.closest("button[data-v]");
    if (!btn) return;
    state.injectMode = btn.dataset.v;
    syncInjectModeUI();
  });
  $("#seg-translate-lang").addEventListener("click", (e) => {
    const btn = e.target.closest("button[data-v]");
    if (!btn) return;
    state.translateLang = btn.dataset.v || "";
    syncTranslateLangUI();
    if (!state.translateLang) {
      // 번역 없음으로 돌리면 기존 번역문은 유지하되 실행만 막음
      renderBlocks();
    }
  });
  $("#btn-run-translate").addEventListener("click", requestTranslate);
  $("#btn-translate-cancel").addEventListener("click", () =>
    $("#translate-overlay").classList.add("hidden"));
  $("#btn-translate-confirm").addEventListener("click", doTranslate);
