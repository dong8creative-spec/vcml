...
const state = {
  ...
  clipCuts: [],             // [{start_us, end_us, text}] — 삽입 시 영상까지 리플 삭제할 구간
};
...
function renderClipCutNote() {
  const note = $("#clip-cut-note");
  if (!note) return;
  const n = state.clipCuts.length;
  if (!n) { note.classList.add("hidden"); note.innerHTML = ""; return; }
  const totalUs = state.clipCuts.reduce((s, c) => s + (c.end_us - c.start_us), 0);
  note.classList.remove("hidden");
  note.innerHTML =
    `✂ 삽입할 때 <b>${n}개 구간</b>의 영상 클립도 함께 삭제됩니다 ` +
    `(약 ${(totalUs / 1e6).toFixed(1)}초, 뒤 전체 당김). ` +
    `<button type="button" class="link-btn" id="btn-undo-clip-cuts">되돌리기</button>`;
  $("#btn-undo-clip-cuts").addEventListener("click", () => {
    state.clipCuts = [];
    renderClipCutNote();
    toast("영상 클립 삭제 예약을 모두 취소했어요. (자막 블록은 복구되지 않아요)", "");
  });
}
  ...
  state.blocks.forEach((b, i) => {
    ...
    row.innerHTML = `
      ...
      <span class="block-btns">
        ...
        <button class="icon-btn del" title="자막만 삭제">✕</button>
        <button class="icon-btn cut" title="자막 + 이 구간 영상 클립 삭제 (뒤 전체 당김)">✂</button>
      </span>`;
    ...
    row.querySelector(".cut").addEventListener("click", () => {
      state.clipCuts.push({ start_us: b.start_us, end_us: b.end_us, text: b.text });
      state.blocks.splice(i, 1);
      renderBlocks();
      scheduleSyncEditorBlocks();
      toast("이 구간 영상 클립은 삽입할 때 함께 삭제돼요.", "");
    });
...
async function doInject() {
  ...
  try {
    const res = await state.api.inject(
      ...
      state.selectedProject.index, state.injectMode, state.clipCuts);
    if (!res.ok) { toast(res.error, "error"); return; }
    let msg = `「${res.project}」에 자막 ${res.count}개를 삽입했어요.`;
    if (res.clip_cuts) {
      msg += ` 영상 클립 ${res.clip_cuts}개 구간`
        + `(약 ${((res.removed_us || 0) / 1e6).toFixed(1)}초)도 리플 삭제했어요.`;
    }
...
function makeMockApi() {
  ...
  return {
    get_state: async () => ({
      ok: true,
      app: { name: "타닥싱크 3", version: "2.16.0-mock" },
