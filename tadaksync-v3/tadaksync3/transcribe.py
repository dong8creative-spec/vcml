...
class Transcriber:
    ...
    def _cpu_options(self) -> dict:
        cores = os.cpu_count() or 4
        # CPU large-v3는 스레드 수를 명시해야 ctranslate2가 보수적으로 잡히는
        # 환경에서도 충분히 병렬화된다. 너무 높이면 UI가 버벅여 상한을 둔다.
        return {
            "cpu_threads": max(2, min(cores, 8)),
            "num_workers": 1,
        }
    ...
    def _load_whisper_model(self, path: str, progress: Callable[[str], None]):
        ...
        self._device = "cpu"
        return WhisperModel(path, device="cpu", compute_type="int8", **self._cpu_options())
...
def audio_has_speech_fast(audio: np.ndarray) -> bool:
    """v3 빠른 사전 게이트. Silero VAD 전체 패스를 생략해 CPU 대기 시간을 줄인다."""
    if audio is None or len(audio) == 0:
        return False
    env = _rms_envelope(audio)
    audio_dur = len(audio) / SR
