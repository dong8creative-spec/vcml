"""로컬 번역 엔진 — Argos Translate로 블록별 번역 (완전 오프라인, 로그인·코인 불필요)."""
...
def _get_argos_translation(from_code: str, to_code: str):
    ...
    try:
        ...
        raise RuntimeError(
            "번역 패키지가 없어요. tadaksync2 폴더에서 "
            "`pip install -r requirements.txt` 후 "
            "`python -m tadaksync2.translate_local --install` 을 실행해 주세요."
        ) from e
    ...
    if not from_lang or not to_lang:
        raise RuntimeError(
            f"번역 언어팩이 없어요 ({from_code}→{to_code}). "
            "한 번만 인터넷에 연결한 뒤 "
            "`python -m tadaksync2.translate_local --install` 을 실행해 주세요."
        )
    ...
    if tr is None:
        raise RuntimeError(
            ...
            "`python -m tadaksync2.translate_local --install` 로 언어팩을 설치해 주세요."
        )
...
if __name__ == "__main__":
    ...
    if "--install" in sys.argv:
        ...
        print("사용법: python -m tadaksync2.translate_local --install")
