...
Cloud Run 운영 시 503·OOM 방지: faster-whisper(small) CPU+int8 도 메모리 스파이크가 크다.
  ...
  VCML_MAX_CONCURRENT_TRANSCRIBES  인스턴스당 동시 전사(Whisper) 개수 상한(기본 1). 초과 시 POST /api/jobs 는 429.
...
정확도 관련 환경 변수(예시):
  ...
  MAX_UPLOAD_MB                        단일 파일 업로드 상한(MB). 로컬 기본 500. Google Cloud Run(완전관리)은
                                         HTTP 요청 본문 약 32MiB 제한이 있어, Run 배포 시 28~30 이하 권장(멀티파트 여유).
...
미리보기: 완료 시 원본 파일 사본이 SUBTITLE_PREVIEW_DIR(기본: 임시 폴더/subtitle_web_preview)에 저장됩니다.
  ...
  편집 화면에서 자막 파일을 저장한 뒤 DELETE /api/jobs/{job_id} 또는 GET …/download 응답 후 서버가 해당 작업·미리보기 사본을 제거합니다.
...
배포(Docker·Cloud Run 등)에서 IP별 10회가 배포마다 초기화되지 않게 하려면:
  컨테이너 로컬 디스크는 휘발되므로, 영구 볼륨을 마운트한 디렉터리를 지정하세요.
  VCML_QUOTA_DATA_DIR — 할당량 SQLite 두 파일(usage_limit.sqlite3, ip_transcription_quota.sqlite3)을
    이 디렉터리에 둡니다. 비우면 기존처럼 홈(~/.local/share/… 등)을 씁니다.
  VCML_IP_QUOTA_DB — IP 전용 DB 파일의 절대 경로(선택). 지정 시 IP 쪽만 이 경로를 우선합니다.

VCML 메인(index) 연동 — 자막 자동생성 메뉴·접속 코드·IP별 할당량:
  VCML_SUBTITLE_GATE=1|0     기본 1. 1이면 접속 코드·쿠키 인증 후에만 자막 앱·API 사용.
  VCML_SUBTITLE_ENABLED=1|0  기본 1. 0이면 자막 앱·API 전체 중단(503, “서비스 이용 불가” 안내 HTML).
  VCML_ACCESS_CODE           기본 0625. POST /api/subtitle-gate 의 code 와 일치해야 함.
  ...
  VCML_JOBS_SQLITE           (선택) 자막 작업 상태 SQLite 경로. 기본 $TMPDIR/vcml_subtitle_jobs.sqlite3.
                              ...
                              IP 할당량은 VCML_QUOTA_DATA_DIR(또는 VCML_IP_QUOTA_DB)로 별도 영구 경로에 두는 것을 권장.
...
_transcribe_sem_init_lock = threading.Lock()

# 작업 ID는 uuid.uuid4().hex (32자리 16진) 만 허용 — 잘못된 URL·캐시 깨짐 시 원인 구분용
_JOB_ID_RE = re.compile(r"^[a-f0-9]{32}$")


def _transcribe_slot_semaphore() -> threading.Semaphore:
    """인스턴스 내 동시 Whisper 전사 상한(기본 1). OOM·CPU 스래싱 완화."""
    global _transcribe_sem
    if _transcribe_sem is not None:
        return _transcribe_sem
    with _transcribe_sem_init_lock:
        if _transcribe_sem is None:
            try:
                raw = (os.environ.get("VCML_MAX_CONCURRENT_TRANSCRIBES") or "1").strip() or "1"
                n = int(raw)
            except ValueError:
                n = 1
            _transcribe_sem = threading.Semaphore(max(1, n))
    return _transcribe_sem
...
def _quota_data_dir_from_env() -> Optional[Path]:
    raw = (os.environ.get("VCML_QUOTA_DATA_DIR") or "").strip()
    if not raw:
        return None
    return Path(raw)
...
def _usage_limit_db_path() -> Path:
    qdir = _quota_data_dir_from_env()
    if qdir is not None:
        return qdir / "usage_limit.sqlite3"
    home = Path.home()
    if os.name == "nt":
...
def _ip_quota_db_path() -> Path:
    ...
    qdir = _quota_data_dir_from_env()
    if qdir is not None:
        return qdir / "ip_transcription_quota.sqlite3"
...
def _subtitle_service_enabled() -> bool:
    """자막 웹앱·API 전체 제공 여부. 0이면 모든 요청에 503(또는 안내 HTML)."""
    v = (os.environ.get("VCML_SUBTITLE_ENABLED") or "1").strip().lower()
    return v not in ("0", "false", "no", "off")


def _subtitle_closed_html() -> str:
    return """<!DOCTYPE html>
<html lang="ko">
<head>
<meta charset="utf-8"/>
<meta name="viewport" content="width=device-width,initial-scale=1"/>
<title>VCML 자막 — 중단</title>
<style>
body{font-family:system-ui,sans-serif;max-width:28rem;margin:3rem auto;padding:0 1.25rem;line-height:1.5;color:#222}
h1{font-size:1.25rem}
</style>
</head>
<body>
<h1>서비스 중단</h1>
<p>자막 자동생성 기능은 현재 제공하지 않습니다.</p>
</body>
</html>"""
...
@app.middleware("http")
async def vcml_subtitle_service_disabled_middleware(request: Request, call_next):
    """VCML_SUBTITLE_ENABLED=0 이면 자막 앱·API 전부 503 (게이트보다 먼저 적용되도록 나중에 등록)."""
    if _subtitle_service_enabled():
        return await call_next(request)
    if request.method == "OPTIONS":
        return await call_next(request)
    accept = (request.headers.get("accept") or "").lower()
    wants_html = "text/html" in accept or (request.headers.get("sec-fetch-mode") or "").lower() == "navigate"
    if request.method == "GET" and wants_html:
        return HTMLResponse(content=_subtitle_closed_html(), status_code=503)
    return JSONResponse(
        {"detail": "자막 자동생성 서비스는 현재 이용할 수 없습니다."},
        status_code=503,
    )
...
def _purge_job_storage(job_id: str) -> None:
    """단일 작업: SQLite jobs 행 삭제 + preview_media_root/job_id 디렉터리 삭제(오디오 사본 등)."""
...
async def subtitle_gate(request: Request, code: str = Form(...)):
    ...
    expected = (os.environ.get("VCML_ACCESS_CODE") or "0625").strip()
...
def _max_upload_mb() -> int:
    try:
        n = int((os.environ.get("MAX_UPLOAD_MB") or "500").strip() or "500")
    except ValueError:
        n = 500
    return max(1, n)
...
def api_transcription_quota(request: Request):
    """자막 자동 생성(POST /api/jobs) 할당량 스냅샷. UI 카운터·업로드 한도 안내용."""
    snap = transcription_quota_snapshot(request)
    snap["max_upload_mb"] = _max_upload_mb()
    return snap
...
def _job_worker(
    ...
    try:
        ...
        try:
            ...
            pass
        _transcribe_slot_semaphore().release()
...
async def create_job(
    ...
    sem = _transcribe_slot_semaphore()
    if not sem.acquire(blocking=False):
        raise HTTPException(
            status_code=429,
            detail="지금은 다른 전사가 진행 중입니다. 잠시 후 다시 시도해 주세요.",
        )
    try:
...
def delete_job_and_preview(job_id: str):
    """편집 완료·파일 저장 후: 서버 작업 메타·미리보기용 원본 사본 제거."""
    job_id = _validate_job_id(job_id)
...
def download_job_result(job_id: str, background_tasks: BackgroundTasks):
    ...
    background_tasks.add_task(_purge_job_storage, job_id)
    return Response(
        content=body or b"",
        media_type=media,
