...
MODEL = "base"  # 체험판: Whisper 2단계
MODEL_BYTES = 145_000_000
LANGUAGE_CHOICES = {
    "자동 감지": None,
    "한국어": "ko",
    "일본어": "ja",
...
class Transcriber:
    ...
    def _ensure_downloaded(self, model_size: str,
            ...
            while not stop.wait(2.0):
                try:
                    ...
                    progress(f"Whisper 모델을 다운로드하고 있어요... {pct:.0f}% "
                             f"(체험판 base, 최초 1회만 받아요)")
                except OSError:
                    pass

        progress("Whisper 모델을 다운로드하고 있어요... (체험판 base, 최초 1회만 받아요)")
        ...
        try:
            ...
            raise RuntimeError(
                "Whisper 모델 다운로드에 실패했어요. 인터넷 연결을 확인한 뒤 "
                "prepare_bundle.ps1 로 모델을 폴더에 넣어 주세요."
            ) from e
        finally:
            stop.set()
